!
!
!     Like t_fcat02, but use 
!        getc, putc 
!     instead of 
!        fread, fwrite  
!
!     It work correcly with Zip file (binary).  
!
      program t_fcat06 
!
      use my_funx
...
!     integer(kind=ck_size_t)    :: lm, lt, m 
      ...
      integer(kind=ck_intptr_t)  iptr_i, iptr_o 
!
!
...
!     if ( errno .ne. 0 ) goto 700 
      ...
      call  funx_fopen ( &
            iptr_i, fnam_i(1:lentrim_fnam_i), "r", errno )
      if ( iptr_i .eq. 0 ) goto 700 
...
!        if ( errno .ne. 0 ) goto 810 
         ...
         call  funx_fstdout ( iptr_o ) 
         if ( iptr_o .eq. 0 ) goto 810 
...
!        if ( errno .ne. 0 ) goto 800 
         ...
         call  funx_fopen ( &
               iptr_o, fnam_o(1:lentrim_fnam_o), "w", errno ) 
         if ( iptr_o .eq. 0 ) goto 800 
...
!        if ( errno .ne. 0 ) goto 701 
         ...
         call  funx_fseek ( iptr_i, offset, whence, errno )
         if ( errno .ne. 0 ) goto 701 
...
!
            call  funx_fseek ( iptr_o, offset, whence, errno )
            if ( errno .ne. 0 ) goto 801
         endif 
!
!
...
!        if ( errno .ne. 0 ) goto 802 
         ...
         do i = 1, lt 
            call funx_putc ( iptr_o, str(i:i), errno )
            if ( errno .eq. ck_eof ) goto 802 
         enddo
!
!
         if ( lt .eq. lm ) then
...
!200  continue
 ...
 500  call  funx_fclose ( iptr_i, errno )
      call  funx_perror ( 'f_fclose: src', errno )

      if ( lentrim_fnam_o .gt. 0 ) then
         call  funx_fclose ( iptr_o, errno )
         call  funx_perror ( 'f_fclose: des', errno )
      endif 
...
!802  call  funx_perror ( 'funx_fwrite_astr', errno )
 802  call  funx_perror ( 'c_putc', errno )
      goto 500 
